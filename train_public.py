...
class ReferenceDetectionDataset(Dataset):
    ...
    def _map_object_id_for_templates(self, object_id):
        """Map search image filename to template category ID.
        
        Examples:
            blacbox_0_drone_video-0011 -> BlackBox_0
            CardboardBox_1_drone_video-0040 -> CardboardBox_1
            lifejacket_0_drone_video-0005 -> LifeJacket_0
            neg_drone_video-0021 -> None (no template for negatives)
        """
        # Handle negatives (no templates)
        if object_id.startswith('neg_'):
            return None
        
        # Map blacbox -> BlackBox
        if object_id.startswith('blacbox_0'):
            return 'BlackBox_0'
        elif object_id.startswith('blacbox_1'):
            return 'BlackBox_1'
        
        # Map lifejacket -> LifeJacket
        elif object_id.startswith('lifejacket_0'):
            return 'LifeJacket_0'
        elif object_id.startswith('lifejacket_1'):
            return 'LifeJacket_1'
        
        # CardboardBox already matches template naming
        elif object_id.startswith('CardboardBox_0'):
            return 'CardboardBox_0'
        elif object_id.startswith('CardboardBox_1'):
            return 'CardboardBox_1'
        
        # Fallback: try extracting category_instance pattern
        # e.g., "SomeObject_0_frame_123" -> "SomeObject_0"
    ...
    def _build_samples(self):
        ...
        for label_file in sorted(label_dir.glob('*.txt')):
            ...
            # Extract object_id from filename (full stem)
            object_id = label_file.stem
            
            # Map to template_id for template loading
            template_id = self._map_object_id_for_templates(object_id)
    ...
    def _load_templates(self, template_id):
        """Load 3 template images for the given template_id.
        
        For zlai_test dataset:
            Pattern: templates/{template_id}_img_001.jpg, {template_id}_img_002.jpg, ...
            Example: BlackBox_0_img_001.jpg, CardboardBox_1_img_001.jpg
        
        For negatives (template_id=None):
            Returns 3 dummy zero tensors (no template needed)
        """
        # Handle negatives: return dummy templates
        if template_id is None:
            dummy = torch.zeros(3, self.template_size[1], self.template_size[0])
            return [dummy, dummy, dummy]
        
        templates_root = self.data_root / self.split / 'templates'
        
        # Load templates matching pattern: {template_id}_img_*.jpg|png
        template_files = sorted(templates_root.glob(f'{template_id}_img_*.jpg')) + \
                        sorted(templates_root.glob(f'{template_id}_img_*.png'))
        
        # If no templates found, raise error
        if len(template_files) == 0:
            raise ValueError(f"No template files found for template_id='{template_id}' in {templates_root}")
    ...
    def __getitem__(self, idx):
        ...
        # Load templates using template_id (not object_id)
        templates = self._load_templates(sample['template_id'])
        ...
        target = {
            ...
            'template_id': sample['template_id']  # for debugging
        }
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'use_augmentation': False,  # Enable/disable data augmentation (True=augment, False=no augment)
